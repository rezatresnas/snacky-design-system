...
// Required by CC BY 4.0 section 3(a): the attribution has to travel with the
// artwork, so it is emitted into every generated file rather than living only
// in NOTICE (which a bundler would strip). `/*!` keeps it through minifiers.
...
const attributionBlock = (prefix) => ATTRIBUTION_LINES.map((l) => prefix + l).join('\n');
...
// The whole generated block sits between these markers so re-running is
// idempotent. An earlier version tried to find the old block by pattern and
// silently appended a second copy instead of replacing it, which left two
// `const ICON_SET=` declarations and broke the page with a SyntaxError.
const BEGIN = '/* BEGIN generated by scripts/generate-icons.js - do not hand-edit */';
const ENDMARK = '/* END generated by scripts/generate-icons.js */';

let start = html.indexOf(BEGIN);
let tail;
if (start !== -1) {
  const e = html.indexOf(ENDMARK, start);
  if (e === -1) throw new Error('index.html: found BEGIN marker without END marker');
  tail = e + ENDMARK.length;
} else {
  // first run on a file that predates the markers: bracket-match the literal
  start = html.indexOf('const ICONS={');
  ...
  tail = end + 1;
  if (html[tail] === ';') tail++;
  // consume a pre-marker ICON_SET block and its alias run, if present
  const legacy = /^\s*(?:\/\/[^\n]*\n)*\s*const ICON_SET=[^\n]*\n(?:ICONS\["[^"]+"\]=ICONS\["[^"]+"\];)*/.exec(
    html.slice(tail)
  );
  if (legacy) tail += legacy[0].length;
}
