...
- `DESIGN.md` - generated by `scripts/generate-design-md.js` from `tokens.json` and
  ...
  (YAML front matter of design tokens + a fixed-order prose body), which Stitch and
  other AI design tools are starting to consume directly, including via a repo URL
  rather than a pasted prompt. Every value in it is looked up from the same token/
  component data the rest of the system already resolves through - colors and radii
  are matched back to their token name by hex/pixel value, never typed by hand - so
  it cannot state a value the rest of the system disagrees with. Re-run after either
  generator file above changes:
  ```
