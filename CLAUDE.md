...
- `DESIGN.md` - generated by `scripts/generate-design-md.js` from `tokens.json` and
  `components.json` (run `generate-agent-files.js` first if those changed). Follows
  Google's open [DESIGN.md format](https://github.com/google-labs-code/design.md)
  (YAML front matter of design tokens + a fixed-order prose body). Kept as a
  generic open-format artifact, not pointed at any specific consumer tool: Google
  Stitch's own "paste/import a DESIGN.md" feature (PREVIEW status) was tested
  directly against this exact file, both pasted raw and via its GitHub-repo field,
  and ignored it completely - wrong colors (invented a "Tertiary" blue this
  system doesn't have, mislabeled the danger red as "Secondary"), wrong font
  (not Poppins, despite the file stating it eleven times). Do not add copy
  inviting anyone toward Stitch specifically for this reason; none of the
  no-file-access docs (`llms.txt`, `design-system-prompt.md`, `README.md`,
  `index.html`'s AI Agent Files modal) name it anymore either. Every value in
  `DESIGN.md` is looked up from the same token/component data the rest of the
  system already resolves through - colors and radii are matched back to their
  token name by hex/pixel value, never typed by hand - so it cannot state a
  value the rest of the system disagrees with, regardless of whether any given
  consumer tool reads it correctly. Re-run after either generator file above
  changes:
  ```
  node scripts/generate-design-md.js
  ```
  Validate with the format's own linter before committing a hand edit to the
  generator (never hand-edit `DESIGN.md` itself):
  ```
