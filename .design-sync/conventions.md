...
Real token families (180 defined total):
...
**Brand font.** All type tokens point at `"Poppins"`, but the package does not ship the
font file - the host app is expected to load it (e.g. a Google Fonts `<link>` for
`Poppins:wght@400;500;600;700;800`). Include that link in any design that uses this
system's typography, or text renders in a fallback serif/sans font.
